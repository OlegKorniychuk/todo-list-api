...
Lists are always returned ordered by `createdAt` ascending.
...
`position` is a fractional sort key managed by the server; clients never set it
directly (see the reorder endpoint below).
...
List tasks in the list. Owner or viewer. Tasks are always returned ordered by
`position` ascending, with `createdAt` then `id` ascending as a tie-breaker
for equal positions.
...
#### `PATCH /lists/:listId/tasks/:taskId/position`

Reorder a task within its list. Owner only.

Request:

```json
{ "afterTaskId": "uuid-of-sibling-task" }
```

Pass `afterTaskId: null` (or omit the field) to move the task to the top of
the list. The server computes a new fractional `position` between the
referenced task and its current successor — clients never supply a raw
position value.

`200 OK` → updated task resource. Errors: `400` `afterTaskId` references the
task itself, `403` viewer, `404` task not found or `afterTaskId` not found in
this list.
...
erDiagram
    ...
    tasks {
        ...
        double precision position  "not null, fractional sort key"
        timestamptz created_at     "not null, default now()"
        timestamptz updated_at     "not null, default now()"
    }
...
- **`tasks`** — composite index `(list_id, position)` for the ordered `GET /lists/:id/tasks`; composite `(list_id, status)` for the `?status=` filtered variant. New tasks append via `position = COALESCE(MAX(position), 0) + 65536`; reordering computes a midpoint between neighbors, falling back to a full-list renumber (`1×gap`, `2×gap`, …) if float precision between two neighbors is exhausted.
