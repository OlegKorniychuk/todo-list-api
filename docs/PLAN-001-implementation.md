...
- [x] **Task 1: Password hashing utility**
  **Description:** Add `bcryptjs` (bundles its own types, no `@types/*` needed). Small wrapper
  ...
  is enough.
  **Acceptance criteria:**
  - [x] `hashPassword`/`verifyPassword` round-trip correctly (hash then verify returns true; wrong
        password returns false)
  - [x] `docker compose build` still succeeds
  **Verification:** `npm test -- crypto`; `docker compose build`
  **Dependencies:** None
  **Files:** `package.json`, `Dockerfile`, `src/common/crypto/password.ts` (+ `.spec.ts`)
  **Scope:** XS

- [x] **Task 2: `UsersService` real implementation**
  ...
  **Acceptance criteria:**
  - [x] `create` inserts and returns the new user; duplicate email throws a `ConflictException` (409)
  - [x] `findByEmail('Foo@Bar.com')` finds a user stored as `foo@bar.com`
  - [x] `findById`/`findByEmail` return `undefined`/`null` (not throw) when not found — callers decide
        the HTTP status
...
- [x] **Task 3: `AuthService.register` + `login`**
  ...
  **Acceptance criteria:**
  - [x] `POST /auth/register` with a fresh email returns `201` + user + token pair; duplicate email
        returns `409`
  - [x] `POST /auth/login` with correct credentials returns `200` + token pair; wrong password or
        unknown email returns `401` (don't leak which one)
  - [x] A row is written to `refresh_tokens` on both register and login
...
- [x] **Task 4: `JwtAuthGuard` + current-user decorator**
  ...
  **Acceptance criteria:**
  - [x] Missing/invalid/expired bearer token → `401`
  - [x] Valid token → handler receives the correct user id via `@CurrentUser()`
...
- [x] **Task 5: `AuthService.refresh` + `logout`**
...
- [x] `npm run build`, `npm run lint:ci`, `npm run format:check`, `npm test` all pass
- [x] `docker compose up -d` then full curl flow: register → login → me → refresh → logout → old
      access token still valid until expiry, old refresh token rejected
- [x] Review with human before starting Phase 2

### Phase 2: Lists

- [x] **Task 7: Shared list-access resolver**
  ...
  **Acceptance criteria:**
  - [x] Owner → `'owner'`; shared user → `'viewer'`; neither → `null`
  - [x] Deleted/nonexistent list → `null` (caller maps to `404`)
...
- [x] **Task 8: `ListsService.create` / `findAllForUser` / `findOne` + guard wiring**
  ...
  **Acceptance criteria:**
  - [x] `POST /lists` → `201`, caller is owner
  - [x] `GET /lists` returns only lists the caller owns or is shared on
  - [x] `GET /lists/:id` → `403` if resolver returns `null`, `404` if list truly doesn't exist
...
- [x] **Task 9: `ListsService.rename` / `remove`**
  **Description:** Owner-only. Non-owner (including viewers) gets `403`.
  **Acceptance criteria:**
  - [x] Owner renames → `200`; viewer or stranger attempts → `403`
  - [x] Owner deletes → `204`; cascades tasks + shares (already enforced at the DB level by the FKs —
...
- [x] Full owner CRUD on lists works end-to-end (curl or a `lists.e2e-spec.ts` if written)
- [x] `npm test` green

### Phase 3: Tasks

- [x] **Task 10: `TasksService.create` / `findAll` / `findOne`**
  ...
  **Acceptance criteria:**
  - [x] Owner creates a task → `201`, defaults `status: 'todo'`
  - [x] Owner or viewer can list/get tasks; a user with no access → `403`
  - [x] `?status=` filter on list works
...
- [x] **Task 11: `TasksService.update` / `updateStatus` / `remove`**
  **Description:** Owner-only for all three. Viewer attempting any → `403`.
  **Acceptance criteria:**
  - [x] Owner edits title/description → `200`; viewer attempt → `403`
  - [x] Owner updates status through all three enum values → `200`; invalid status value → `400`
        (already enforced by `UpdateTaskStatusDto`'s `@IsIn` — confirm it's actually reached)
  - [x] Owner deletes → `204`; viewer attempt → `403`
  **Verification:** `npm test -- tasks.service`
...
- [x] Full task CRUD + status transitions work for an owner; `npm test` green

### Phase 4: Sharing

- [x] **Task 12: `SharesService.create`**
  **Description:** Owner-only. Resolve target by email via `UsersService.findByEmail` (404 if no such
  ...
  request body yet).
  **Acceptance criteria:**
  - [x] Owner shares with an existing user's email → `201`
  - [x] Sharing with own email → `422`; sharing with unknown email → `404`; duplicate share → `409`
  ...
  - [x] Non-owner attempting to share → `403`
...
- [x] **Task 13: `SharesService.findAll` / `remove`**
  **Description:** Owner-only listing and revocation.
  **Acceptance criteria:**
  - [x] Owner lists shares → `200` with resolved emails; non-owner → `403`
  - [x] Owner revokes a share → `204`; revoked user immediately loses list/task read access
  **Verification:** `npm test -- shares.service`
...
- [x] `npm run build && npm run lint:ci && npm run format:check && npm test` all green
...
- [x] Every error code in `docs/SDD-001.md` (`400/401/403/404/409/422`) has been hit at least once
      (unit tests; not yet a manual curl pass — see item above)

### Phase 5: End-to-end tests

- [x] **Task 14a: Testcontainers e2e harness**
  **Description:** Add `@testcontainers/postgresql` (dev dep). Global Jest setup
  ...
  Nest testing module).
  **Acceptance criteria:**
  - [x] Container starts, migrations apply, `DATABASE_URL` resolves to the container before any spec
        body runs
  - [x] Container is torn down after the suite, pass or fail (no leaked containers)
...
- [x] **Task 14b: Full-flow e2e spec**
  **Description:** Superseded by a per-route error-path suite (user's explicit direction, broader than
  the original single-flow-spec scope): `test/auth.e2e-spec.ts`, `test/lists.e2e-spec.ts`,
  `test/tasks.e2e-spec.ts`, `test/shares.e2e-spec.ts`, each covering the main success + error paths per
  route (not just one happy-path walk). Every test gets its own fresh Testcontainers Postgres instance
  via `beforeEach`/`afterEach` (see `test/utils/e2e-app.ts`, `test/utils/fixtures.ts`) instead of one
  shared container per run — full isolation, no cross-test state. Surfaced and fixed a real bug along
  the way: `isUniqueViolation` (users + shares services) checked `err.code` directly, but the real
  `DrizzleQueryError` nests the pg error under `.cause` — unit tests mocked a flat shape that never
  matched production, so `409` responses were 500ing until e2e caught it. Fixed via a shared
  `src/common/db/pg-error.ts` helper (+ `.spec.ts`) that checks both shapes.
  **Acceptance criteria:**
  - [x] `npm run test:e2e` passes against the Testcontainers-provisioned database
  - [x] Every status code in the SDD error table (`400/401/403/404/409/422`) is exercised at least
        once across the suite
  **Verification:** `npm run test:e2e`
  **Dependencies:** Task 14a
  **Files:** `test/auth.e2e-spec.ts`, `test/lists.e2e-spec.ts`, `test/tasks.e2e-spec.ts`,
  `test/shares.e2e-spec.ts`, `test/utils/e2e-app.ts`, `test/utils/fixtures.ts`,
  `src/common/db/pg-error.ts` (+ `.spec.ts`)
  **Scope:** M
